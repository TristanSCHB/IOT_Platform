...
|----------|------------|------|
| **Serveur** | **VM Linux** | Héberge le subscriber MQTT, traite les données des capteurs et les stocke dans la base de données. |
...
| **Raspberry Pi / VM Linux** | **Mosquitto (MQTT Broker)** | Sert de passerelle entre la carte Nucleo (via Bluetooth) et le serveur. Les messages MQTT sont publiés sur des topics tels que `sensors/#`. |
